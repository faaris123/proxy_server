...
void parse_client_request(int fd, struct QueueNode *node)
{
    char *read_buffer = malloc(LIBHTTP_REQUEST_MAX_SIZE + 1);
    node->req_str = malloc(LIBHTTP_REQUEST_MAX_SIZE + 1);
    ...
    strncpy(node->req_str, read_buffer, LIBHTTP_REQUEST_MAX_SIZE + 1);
